...
class MaximumSumCombination {
	...
	private static void solve(Integer[] a, Integer[] b, int n, int k) {
		...
		for (int i = 0; i < k; i++) {
			...
			// already present inside set i.e. no repeating pair should be present
			// inside the heap.
			if (l >= 0 && m >= 0 && !pairs.contains(new Pair(l, m))) {
...
// Brute force with Tc as O(n2 * log(n2)) and Sc as O(n2)
